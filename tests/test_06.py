...
image = cv2.imread("d:\\temp\\human2.png")
...
#metadata=MetadataCatalog.get(cfg.DATASETS.TRAIN[0])
[result.extend((#x,
       output["instances"][x].pred_classes.item(),      #class_id
       [metadata.thing_classes[x] for x in output["instances"][x].pred_classes][0], #class
       output["instances"][x].scores.item(),     
       output["instances"][x].pred_boxes.tensor.cpu().numpy()[0][0],
       output["instances"][x].pred_boxes.tensor.cpu().numpy()[0][1],
       output["instances"][x].pred_boxes.tensor.cpu().numpy()[0][2],
       output["instances"][x].pred_boxes.tensor.cpu().numpy()[0][3]) 
for x in range(len(output["instances"])))]
