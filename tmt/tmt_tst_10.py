...
#for name in ["s-IMG_2973"]:
    im = cv2.imread(f"./tmt/tomato/test/{name}.jpg")
    ...
    vis = Visualizer(im[:, :, ::-1], metadata=tmt_metadata, instance_mode=ColorMode.IMAGE_BW, scale=1.0)
    vis.draw_text(stnum, (0,0), horizontal_alignment="left")

    bFlag = 1
    if bFlag:
        v = vis.draw_instance_predictions(outputs["instances"].to("cpu"))   #VisImageを取得
    else:
        #
        # Boxes
        ...
        for mask in outputs["instances"].pred_masks.to('cpu'):
            vis.draw_soft_mask(mask)    #   Visualizerにmaskを描画
        v = vis.get_output()            #   VisImageを出力

    #結果画像表示
    cv2.imshow(f"{ver}_{name}.jpg", v.get_image()[:, :, ::-1])  #   v : VisImageを表示
